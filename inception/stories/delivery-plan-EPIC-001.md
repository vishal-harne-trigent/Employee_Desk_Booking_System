...
**One story = one PR** (`feat/US-###-<slug>`). Rows below are **agent runtime to complete each story’s implementation** — not human developer or QA person-days.
...
| **B — Auth + booking** | US-001, US-002 | 13 | 3–4.25 h | 0.25–0.4 h | 1.5–2 h | — | **4.75–6.65 h** |
| **C — Employee** | US-003, US-009 | 8 | 2.25–3 h | — | 1.25–1.75 h | — | **3.5–4.75 h** |
| **D — Admin** | US-004, US-005, US-006 | 18 | 4.25–6 h | 0.25–0.4 h | 2–3 h | — | **6.5–9.4 h** |
| **E — Notifications** | US-007, US-008 | 13 | 3.25–4.25 h | 0.25–0.4 h | 1.75–2.5 h | — | **5.25–7.15 h** |
| **Sprint 1 total** | Full MVP | **52** | **12–18 h** | **1.15–1.9 h** | **7–10 h** | **0.75–1.5 h** | **~21–31 h agent** |

Test project scaffold and AC test naming for **`/qa`** are part of Phase A exit (CI + test shell) — agent time for tests is in the **`/qa`** column, not **`/dev`** implementation subtotal alone.
...
Phase B–E (stories, dependency order):
  US-001 → US-002 → US-003 → US-009 → US-004 → US-005 → US-006 → US-007 ∥ US-008
  (/qa after each story merge per ai/gates/delivery.md)
```
...
Human D1/D2 gates and **`/qa`** test agent sessions sit **outside** the **`/dev`**-only story subtotal (included in Sprint 1 phase table).
...
**Next (agents):** Phase A — `/architect` + `/devops` **~1.25–2.25 h agent**, then story agents US-001 onward in dependency order.
